            ...
            <div class="form-row">
                ...
                <div class="form-group col-12 col-md-4">
                    ...
                    <select name="statuses[]" id="statuses" style="height:48px" multiple>
                        <option value="{{ \App\Enums\UserStatus::Pending }}" {{ in_array(\App\Enums\UserStatus::Pending, ( request('statuses') ?? [] )) ? 'selected' : '' }}>{{ trans('statuses/user.' . \App\Enums\UserStatus::Pending) }}</option>
                        <option value="{{ \App\Enums\UserStatus::Active }}" {{ in_array(\App\Enums\UserStatus::Active, ( request('statuses') ?? [] )) ? 'selected' : '' }}>{{ trans('statuses/user.' . \App\Enums\UserStatus::Active) }}</option>
                        <option value="{{ \App\Enums\UserStatus::Ban }}" {{ in_array(\App\Enums\UserStatus::Ban, ( request('statuses') ?? [] )) ? 'selected' : '' }}>{{ trans('statuses/user.' . \App\Enums\UserStatus::Ban) }}</option>
                    </select>
                </div>
        ...
        <div class="float-left table-filters">
            ...
            <a href="?garbage_status=un-deleted"
               class="page-link d-inline-block {{ request()->get('garbage_status') == 'un-deleted' ? 'bg-dark text-light shadow' : 'text-dark' }}">حذف
                نشده</a>
            <a href="?statuses[]={{ \App\Enums\UserStatus::Pending }}"
               class="page-link d-inline-block {{ in_array(\App\Enums\UserStatus::Pending, (request()->get('statuses') ?? [])) ? 'bg-warning text-white shadow' : 'text-dark' }}">
                {{ trans('statuses/user.' . \App\Enums\UserStatus::Pending) }}
            </a>
            <a href="?statuses[]={{ \App\Enums\UserStatus::Active }}"
               class="page-link d-inline-block {{ in_array(\App\Enums\UserStatus::Active, (request()->get('statuses') ?? [])) ? 'bg-warning text-white shadow' : 'text-dark' }}">
                {{ trans('statuses/user.' . \App\Enums\UserStatus::Active) }}
            </a>
            <a href="?statuses[]={{ \App\Enums\UserStatus::Ban }}"
               class="page-link d-inline-block {{ in_array(\App\Enums\UserStatus::Ban, (request()->get('statuses') ?? [])) ? 'bg-warning text-white shadow' : 'text-dark' }}">
                {{ trans('statuses/user.' . \App\Enums\UserStatus::Ban) }}
            </a>
            <a href="{{ route('adminPanel.users.index') }}"
               class="page-link d-inline-block {{ request()->fullUrl() == route('adminPanel.users.index') ? 'bg-warning text-white shadow' : 'text-dark' }}">همه</a>
        </div>

        <div class="float-right text-muted">{{ $users->total() }}<span> آیتم دریافت شد</span></div>
    </div>
    ...
    <table class="table table-white table-striped table-hover table-bordered text-center table-responsive-md">
        ...
        @foreach($users as $user)
            <tr>
                ...
                <td>{{ trans('statuses/user.' . $user->getStatus()) }}</td>
                ...
                <td>
                    <a class="btn  btn-icon" href="{{ route('adminPanel.users.edit', $user) }}">
                        <i class="fa fa-edit text-primary"></i>
                    </a>


                    @if($user->trashed())
                        <form action="{{ route('adminPanel.users.recycle', $user) }}"
                    ...
                    @else
                        <form action="{{ route('adminPanel.users.destroy', $user) }}"
                              method="POST" class="d-inline">
                            @csrf
                            @method('DELETE')
